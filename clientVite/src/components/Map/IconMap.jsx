...
const IconMap = () => {
  ...
  useEffect(() => {
    ...
    const handleClick = (event) => {
      ...
      actual.innerHTML = selectedPath.getAttribute("data-name");
    };

    const paths = document.querySelectorAll("path");
    paths.forEach((path) => {
      path.addEventListener("touchstart", handleClick);
      path.addEventListener("click", handleClick);
    });

    return () => {
      paths.forEach((path) => {
        path.removeEventListener("touchstart", handleClick);
  ...
  function handleEnter(event) {
    ...
    setCountry(event.target.attributes[2].nodeValue);
  }
  return (
    <div
      className="flex flex-col border-4 m-auto border-gray-950 w-7/12"
      style={{ position: "relative" }}
    >
      <div className="whitespace-normal">{country}</div>
